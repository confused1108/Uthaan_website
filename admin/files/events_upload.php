...
$fi = new FilesystemIterator("../uploads/events/", FilesystemIterator::SKIP_DOTS);
...
$newFilePath = "../uploads/events/" . "img_event_".$var.".".$extension;
$filename="img_event_".$var.".".$extension;
...
if(move_uploaded_file($tmpFilePath, $newFilePath)) {
    ...
    {
        $sql = "INSERT INTO
                    events(heading,description,organisers,winners,thumbnail,var,date,admin_id)
                VALUES(?,?,?,?,?,?,?,?)";
        $test=$db->prepare($sql);
        $test->bindParam(1,$heading);
        $test->bindParam(2,$description);
        $test->bindParam(3,$organisers);
        $test->bindParam(4,$winners);
        $test->bindParam(5,$filename);
        $test->bindParam(6,$var);
