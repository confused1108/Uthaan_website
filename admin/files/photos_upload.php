...
$target_dir = "../uploads/gallery/photos/";
$fi = new FilesystemIterator("../uploads/gallery/photos/", FilesystemIterator::SKIP_DOTS);
...
for($i=0; $i<$total; $i++) {
    ...
    $newFilePath = "../uploads/gallery/photos/" . "img_pic_".$var.".".$extension;
    $filename="img_pic_".$var.".".$extension;
    ...
    if(move_uploaded_file($tmpFilePath, $newFilePath)) {
        ...
        {
            $sql = "INSERT INTO
                    gallery_images(heading,description,image_name,var,date,admin_id)
                VALUES(?,?,?,?,?,?)";
            $test=$db->prepare($sql);
            $test->bindParam(1,$heading);
            $test->bindParam(2,$description);
            $test->bindParam(3,$filename);
            $test->bindParam(4,$var);
